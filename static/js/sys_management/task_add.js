...
// $("#sys_select .select2_box").select2("val", "W");
...
function confirm() {
    ...
    show_loading2();
    $.post(site_url + "create_task/", {"data": data}, function (res) {
            close_loading2();
            if (res.is_success) {
                if (res.data == 'NOW'){
                    for(var i=0;i<res.result_data[1].length;i++){
                        ip_fail += res.result_data[1][i] + ' '
                    }
                    for(var i=0;i<res.result_data[0].length;i++){
                        ip_sucess += res.result_data[0][i] + ' '
                    }
                    if (res.result_data[0] == '' || res.result_data[0] == undefined){
                        app_none_modal_alert("全部服务器执行失败：" + ip_fail);
                    }
                    else if (res.result_data[1] == '' || res.result_data[1] == undefined){
                        app_none_modal_alert("全部服务器执行成功：" +ip_sucess);
                    }
                    else {
                       app_none_modal_alert("执行成功的服务器：" +ip_sucess + "<br>" + "执行失败的服务器：" +ip_fail);
                    }

                }
                else if (res.data == "TIMER"){
                    window.location.href = site_url + "sys_management/task_report/"
                }
            } else {
                app_alert(res.message);
            }
